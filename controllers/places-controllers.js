...
const getPlacesByUserId = async (req, res, next) => {
  ...
  if (!userPlaces || userPlaces.places.length === 0) {
    return next(
      new HttpError(`Could not find any places for this user...`, 404)
    );
  } else {
    //the mongoose find() method returns places in an array
    //but each place needs its getters to still be set to true
    let places = userPlaces.places.map((place) =>
      place.toObject({ getters: true })
    );

    try {
      for (let place of places) {
        const imageUrl = await getObjectSignedUrl(place.image);
        place.image = imageUrl;
      }
    } catch (err) {
      const error = new HttpError(
        "Error occurred while fetching royal places' data.",
        500
      );
      return next(error);
    }
...
const createPlace = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError(
      "Could not find user for provided ID. Try again please!",
      500
    );
    return next(error);
  }
...
const updatePlace = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError(
      "An error has prevented your royal place from updating",
      500
    );
    return next(error);
  }

  //check if patch request comes from authorized user
  if (place.creator.toString() !== req.userData.userId) {
    const error = new HttpError(
      "You are not allowed to invade (edit) another ruler's place!",
      401
    );
    return next(error);
  }
 ...
 * Also deletes the place's image from AWS S3 Bucket.
 */
const deletePlace = async (req, res, next) => {
  // identify place selected for deletion by placeID in url
  const placeId = req.params.pid;
  ...
  try {
    ...
    const error = new HttpError(
      "An error has prevented the deletion of your place",
      500
    );
    return next(error);
  }
  ...
  //check if delete request comes from authorized user
  if (place.creator.id !== req.userData.userId) {
    const error = new HttpError(
      "You are not allowed to conquer (delete) another ruler's place!",
      401
    );
    return next(error);
  }

  //Deleting the place from Mongo AND image from S3 Bucket
  try {
    //deleting place from MongoDB
    const sess = await mongoose.startSession();
    sess.startTransaction();
